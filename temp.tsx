...
const PATTERNS = [
  { id: 'active-sprint', label: 'Active Sprint', emoji: '≡ƒÜÇ', desc: 'Heavy weekday activity, focused sprints' },
  { id: 'side-project', label: 'Side Project', emoji: '≡ƒîÖ', desc: 'Sporadic bursts, evenings & weekends' },
  { id: 'daily-grind', label: 'Daily Grind', emoji: 'ΓÜÖ∩╕Å', desc: 'Consistent commits every day' },
  { id: 'weekend-warrior', label: 'Weekend Warrior', emoji: '≡ƒÅä', desc: 'Most work on weekends' },
  { id: 'crunch-mode', label: 'Crunch Mode', emoji: '≡ƒöÑ', desc: 'Deadline-driven, late nights' },
  { id: 'casual', label: 'Casual', emoji: 'Γÿò', desc: 'Relaxed, occasional commits' },
]
...
export default function Home() {
  ...
  // ΓöÇΓöÇΓöÇ Render helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  const renderStep1 = () => (
      ...
      >
        ...
        {file ? (
          <div className="animate-fadeIn">
            ...
            {sessionId && <p className="font-mono text-xs text-brand-green/60 mt-2">Γ£ô Uploaded ΓÇö {fileCount} files detected</p>}
  ...
  const renderStep3 = () => (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex items-center justify-between p-3 rounded-xl border border-white/10 bg-white/5">
        ...
        <span className="font-mono text-xs text-brand-green">{fileCount > 0 ? `Γëê${Math.ceil(fileCount / Math.max(dayCount, 1))} commits/day` : ''}</span>
      </div>
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <label className="flex items-center gap-3 cursor-pointer">
        ...
        <div>
          ...
          <p className="font-mono text-xs text-white/25">Adds general merge commits every 5ΓÇô9 commits</p>
        </div>
      </label>
      ...
      <div className="mt-2 pt-4 border-t border-white/5">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          {([
            { id: 'default', label: 'ΓÜû∩╕Å Balanced', map: {} as Record<string, number> },
            { id: 'fullstack', label: '≡ƒûÑ∩╕Å Full-Stack', map: { ts: 50, tsx: 30, css: 10, json: 10 } },
            { id: 'backend', label: 'ΓÜÖ∩╕Å Backend Heavy', map: { ts: 60, js: 20, json: 15, md: 5 } },
            { id: 'frontend', label: '≡ƒÄ¿ Frontend Heavy', map: { tsx: 50, css: 30, ts: 20 } },
    ...
    <div className="space-y-5 animate-fadeIn">
      ...
      {stage === 'done' && result && (
        <div className="space-y-4 animate-fadeIn">
          ...
          <p className="font-mono text-xs text-white/30 text-center">{result.startDate} ΓåÆ {result.endDate}</p>
          ...
          {session?.user && !pushResult && (
            <div className="p-4 rounded-xl border border-white/10 bg-white/5 space-y-3">
              ...
              <button onClick={handlePushToGithub} disabled={pushingToGithub || !repoName}
                ...
                <span className="flex items-center justify-center gap-2">
                  {pushingToGithub ? <><svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeDasharray="30 70" /></svg>Pushing...</> : 'ΓåÆ Push to GitHub'}
                </span>
              </button>
            </div>
          )}
          ...
          <button onClick={() => { setStage('idle'); setResult(null); setVisibleCommits([]) }}
            ...
            Γå║ regenerate with different settings
          </button>
        </div>
      )}
  ...
  // ΓöÇΓöÇΓöÇ Render Landing Page ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  if (status === "loading") {
      ...
      <div className="relative min-h-screen bg-[#050508] flex flex-col items-center justify-between overflow-x-hidden selection:bg-brand-green/30 selection:text-brand-green">
        ...
        <main className="relative z-10 w-full max-w-7xl mx-auto px-6 pt-20 pb-32 flex flex-col items-center text-center">
          ...
          <button onClick={() => signIn('github')} className="group relative inline-flex items-center justify-center gap-4 px-10 py-5 rounded-2xl font-mono text-lg font-bold text-[#050508] transition-all hover:scale-105 active:scale-95 shadow-[0_0_40px_rgba(0,255,135,0.3)] hover:shadow-[0_0_60px_rgba(0,255,135,0.5)] bg-gradient-to-r from-[#00ff87] to-[#00d4ff]">
            ...
            <span className="relative z-10">Sign in with GitHub to start ΓåÆ</span>
          </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 p-8 rounded-3xl border border-white/10 bg-[#0a0a0f]/80 backdrop-blur-xl hover:border-[#00d4ff]/40 transition-colors group relative overflow-hidden">
              ...
              <div className="font-mono text-xs p-4 rounded-xl bg-black/50 border border-white/5 text-[#00d4ff]">
                ...
                [Γ£à] feat(auth): implement JWT token rotation and secure session cookies
              </div>
            </div>
            ...
            <div className="p-8 rounded-3xl border border-white/10 bg-[#0a0a0f]/80 backdrop-blur-xl hover:border-[#b026ff]/40 transition-colors group relative overflow-hidden">
              ...
              <div className="w-12 h-12 rounded-2xl bg-[#b026ff]/10 border border-[#b026ff]/30 flex items-center justify-center mb-6">
                <span className="text-2xl">≡ƒöÇ</span>
              </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-[#050508] shadow-2xl overflow-hidden relative group">
              ...
              <div className="p-6 font-mono text-sm leading-relaxed text-[#00ff87]/70 min-h-[300px] flex flex-col justify-end overflow-hidden relative">
                ...
                <div className="animate-marquee-up space-y-1.5 whitespace-nowrap opacity-90 pb-2">
                  ...
                  <p className="text-[#00ff87]">Γ£ô [db4f1a] feat(auth): implement JWT refresh</p>
                  <p className="text-[#00ff87]">Γ£ô [9a2c3d] fix(ui): resolve overflow on mobile</p>
                  <p className="text-[#00ff87]">Γ£ô [e3b21c] refactor: extract payment hook</p>
                  <p className="text-[#b026ff] mt-2">&gt; Merge branch 'feature/payments-42' into main</p>
                  <p className="text-white mt-4 font-bold">Γ£ô 385 Commits generated across 89 active days.</p>
            ...
            <div className="space-y-8">
              <div className="glass-card p-6 rounded-3xl border border-[#00ff87]/30 bg-[#00ff87]/5 group relative overflow-hidden">
                ...
                <div className="mt-6 flex items-center justify-between font-mono text-xs text-white/50">
                  ...
                  <a href="#" className="text-[#00ff87] hover:underline">Read the Docs ΓåÆ</a>
                </div>
              </div>
    ...
    <div className="relative min-h-screen flex flex-col">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
            ...
            <div className="lg:col-span-3">
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                {step < 5 && (
                  ...
                  >
                    <span>{step === 2 && !sessionId ? 'Upload & Continue' : step === 4 ? 'Review & Generate ΓåÆ' : 'Continue ΓåÆ'}</span>
                  </button>
                )}
              </div>
            </div>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-2">
                      <span className={`font-mono text-xs ${check.ok ? 'text-brand-green' : 'text-white/20'}`}>{check.ok ? 'Γ£ô' : 'Γùï'}</span>
        ...
        <footer className="border-t border-white/5 px-6 py-4">
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            <span className="font-mono text-xs text-subtle">GITTIME v2.0 ΓÇö undetectable commit history</span>
...
// ΓöÇΓöÇΓöÇ Heatmap Component ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
